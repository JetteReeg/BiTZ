...
void RuntimeEnvironment::readSimDef(const string file){
    ...
    ifstream DefFile(file.c_str());
        ...
        while (getline(DefFile, line))
        {
            ...
            ss >> SRunPara::RunPara.weather_std;
            ss >> SRunPara::RunPara.p_dist_arable;
            ss >> SRunPara::RunPara.p_dist_grass;
            ss >> SRunPara::RunPara.p_dist_urban;
            ss >> SRunPara::RunPara.p_dist_forest;
            ss >> SRunPara::RunPara.p_dist_bare;
...
void RuntimeEnvironment::one_year(){
    ...
    for (auto it = GridEnvironment::Patch_defList.begin(); it!=GridEnvironment::Patch_defList.end(); it++){
        ...
        if (tmp->Type=="arable" && probability < SRunPara::RunPara.p_dist_arable){
            PID_disturbed.push_back(tmp->PID);
        }
        // for grassland patches, the probability is 80% for being disturbed within the current year
        if (tmp->Type=="grassland" && probability < SRunPara::RunPara.p_dist_grass){
            PID_disturbed.push_back(tmp->PID);
        }
    }
    ...
    for (unsigned int cell_i=0; cell_i<SRunPara::RunPara.GetSumCells(); ++cell_i){
        ...
        if(!cell->TZ){
            ...
            // if urban
            if (cell->LU_id==4 && combinedLCG() < SRunPara::RunPara.p_dist_urban){
                for (unsigned pop_i=0; pop_i < cell->FT_pop_List.size(); pop_i++){
                    std::shared_ptr<FT_pop> curr_Pop=cell->FT_pop_List.at(pop_i);
                    FT_pop::disturbance(curr_Pop);
                }
            ...
            // if forest
            if (cell->LU_id==2 && combinedLCG() < SRunPara::RunPara.p_dist_forest){
                for (unsigned pop_i=0; pop_i < cell->FT_pop_List.size(); pop_i++){
                    std::shared_ptr<FT_pop> curr_Pop=cell->FT_pop_List.at(pop_i);
                    FT_pop::disturbance(curr_Pop);
                }
            ...
            // if bare
            if (cell->LU_id==0  && combinedLCG() < SRunPara::RunPara.p_dist_bare){
                for (unsigned pop_i=0; pop_i < cell->FT_pop_List.size(); pop_i++){
                    std::shared_ptr<FT_pop> curr_Pop=cell->FT_pop_List.at(pop_i);
                    FT_pop::disturbance(curr_Pop);
                }
...
void RuntimeEnvironment::weather(){
    ...
    //else
    //normal distribution
    eps=normcLCG(0.0,SRunPara::RunPara.weather_std);
