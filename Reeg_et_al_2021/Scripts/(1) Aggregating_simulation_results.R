...
#' CONTENTS: 
#'  - This code is implemented to aggregate the simulation output for 
#'  this manuscript. It expects output files for each landscape simulated. 
#'  Folder structure needs to be as presented in the repository
#'  DEPENDENCIES:
#'  - The code needs the simulation output of the model
#'  
#' AUTHOR: Jette Reeg
#' ####################################################################### #
## required packages ===========================================================
...
require(vegan)
## Working directory ===========================================================
# The working directory needs to be the Simulation folder
setwd("./Simulations")
## Aggregating model output for each landscape =================================
# Go through each subfolder, i.e. each landscape
...
for (dir in directories){
  ...
  if (any(duplicated(scenarios$SimNb))){
      ...
      for (file in 1:length(file_list_Gridout)){
        ...
        if (nrow(data)> max_nrow){
          ...
          for (j in 0:(nb_simulations-1)){
            ...
            NFT_LU.pa<-NFT_LU.pa[,.(Popsize=sum(Popsize)),, by=.(Scenario, MC, Year, FT_ID, LU, LU_ID)]
    ...
    data.list.lu.pa[LID=="1c" | LID=="2c" | LID=="4e", Cluster:=2]
    ...
    data.list.lu[LID=="1c" | LID=="2c" | LID=="4e", Cluster:=2]
    ...
    data.list.land[LID=="1c" | LID=="2c" | LID=="4e", Cluster:=2]
    ...
    NFT.list.lu.pa[LID=="1c" | LID=="2c" | LID=="4e", Cluster:=2]
    ...
    NFT.list.lu[LID=="1c" | LID=="2c" | LID=="4e", Cluster:=2]
    ...
    NFT.list.land[LID=="1c" | LID=="2c" | LID=="4e", Cluster:=2]
...
## Aggregating over all landscapes =============================================
